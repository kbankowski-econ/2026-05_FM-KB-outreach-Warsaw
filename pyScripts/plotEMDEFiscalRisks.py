"""
Plot Fiscal Risks in IMF Staff Reports on EMDEs (Figure 1.34.1).
Horizontal bar chart, same style as plotAEFiscalRisks.
"""
import pandas as pd
...
def main():
    ...
    df = pd.read_excel(xl_path, sheet_name='Figure 1.34.1', header=None, skiprows=1)
    ...
    w_px, h_px = get_chart_dims_px("emde_fiscal_risks.png")
    fig.update_layout(
    ...
    save_path = output_dir / "emde_fiscal_risks"
    ...
    df_out = df.iloc[::-1].reset_index(drop=True)
    df_out.to_csv(f"{save_path}.csv", index=False)
    print(f"EMDE Fiscal Risks chart saved to {save_path}.png")
